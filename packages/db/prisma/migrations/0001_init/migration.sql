...
-- Enforce immutability: no UPDATE, no DELETE, no TRUNCATE on the audit table.
-- =============================================================================

CREATE OR REPLACE FUNCTION audit_logs_prevent_update()
RETURNS TRIGGER AS $$
BEGIN
  RAISE EXCEPTION 'audit_logs is append-only: UPDATE is forbidden (row id=%)', OLD.id
...
RETURNS TRIGGER AS $$
BEGIN
  RAISE EXCEPTION 'audit_logs is append-only: DELETE is forbidden (row id=%)', OLD.id
    USING ERRCODE = 'insufficient_privilege';
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER audit_logs_no_truncate
  ...
  FOR EACH STATEMENT EXECUTE FUNCTION audit_logs_prevent_truncate();

-- Auto-stamp created_at as now() if NULL (audit rows are immutable thereafter)
CREATE OR REPLACE FUNCTION audit_logs_stamp_created_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER audit_logs_stamp
  ...
  FOR EACH ROW EXECUTE FUNCTION audit_logs_stamp_created_at();
